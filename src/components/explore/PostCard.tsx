...
const PostCard: React.FC<PostCardProps> = ({
  ...
  return (
    <div className="p-4 @container border-b border-chip-dark-bg/60">
      <div className="flex gap-4">
        <img className="w-12 h-12 rounded-full object-cover" data-alt={`Profile picture of ${userName}`} src={avatarSrc} />
        <div className="flex w-full flex-col gap-3">
          <div className="flex items-center gap-2">
            <p className="text-white text-base font-bold leading-normal">{userName}</p>
            <p className="text-nav-dark-text text-sm font-normal leading-normal">{userHandle} · {timeAgo}</p>
          </div>
          <p className="text-text-dark text-base font-normal leading-relaxed">{content}</p>
          {mediaImageSrc && mediaTitle && (
            <div className="flex flex-col items-stretch justify-start rounded-xl mt-2 border border-chip-dark-bg">
              <div
              ...
              <div className="flex w-full min-w-72 grow flex-col items-stretch justify-center gap-1 p-4">
                <p className="text-nav-dark-text text-sm font-normal leading-normal">{mediaType === "film" ? "Film" : "Kitap"}</p>
                <p className="text-white text-lg font-bold leading-tight tracking-[-0.015em]">{mediaTitle}</p>
              </div>
            </div>
          )}
          <div className="flex items-center justify-between text-nav-dark-text mt-3">
