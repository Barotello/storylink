...
import SettingsSubPageAppBar from "@/components/settings/SettingsSubPageAppBar"; // AppBar'ı kullanmak için import et

interface ProfilePageProps {
  isDrawer?: boolean; // Çekmece içinde mi açılıyor kontrolü
}
...
const ProfilePage: React.FC<ProfilePageProps> = ({ isDrawer = false }) => {
  ...
  return (
    <div className="relative w-full flex flex-col group/design-root overflow-x-hidden font-display text-text-dark">
      {!isDrawer && (
        <header className="sticky top-0 z-10 flex items-center bg-background-dark/80 p-4 pb-2 backdrop-blur-sm justify-between">
          <Button
            variant="ghost"
            className="flex size-10 shrink-0 items-center justify-center rounded-full transition-colors hover:bg-white/10 p-0"
            onClick={() => navigate(-1)}
          >
            <span className="material-symbols-outlined text-text-dark">arrow_back</span>
          </Button>
          <h1 className="flex-1 text-center text-xl font-bold leading-tight tracking-tight text-text-dark">Profil</h1>
          <Link to="/settings" className="flex size-10 shrink-0 items-center justify-center p-0">
            <Button variant="ghost" className="text-text-dark p-0">
      ...
      <div className="relative w-full h-[60vh] sm:h-[70vh]">
        ...
        <div className="absolute top-4 right-4 z-10">
          <Button className="w-10 h-10 rounded-full bg-background-dark/40 backdrop-blur-sm flex items-center justify-center text-white">
      ...
      <div className="p-5 -mt-20 relative z-10 space-y-6">
        ...
        <div className="bg-surface-dark/20 backdrop-blur-xl p-5 rounded-lg border border-border-dark shadow-lg">
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-white tracking-tight text-3xl font-bold leading-tight">Jessica, 29</h1>
              <p className="text-subtle-dark text-base font-normal leading-normal pt-1">İstanbul, Türkiye • UI/UX Tasarımcısı</p>
            </div>
            <div className="flex items-center gap-2 pt-1">
              <span className="material-symbols-outlined text-green-400 text-sm">circle</span>
              <span className="text-text-dark text-sm">Online</span>
            </div>
          </div>
        </div>
        {/* 3. Hakkında Bölümü */}
        <div className="space-y-2">
          <h3 className="text-text-dark text-lg font-bold leading-tight tracking-[-0.015em] px-1">Hakkında</h3>
          <p className="text-subtle-dark text-base font-normal leading-relaxed px-1">
        ...
        <div className="space-y-4">
          <h3 className="text-text-dark text-lg font-bold leading-tight tracking-[-0.015em]">Favori Filmleri</h3>
        ...
        {/* 5. Kitap Galerisi */}
        <div className="space-y-4">
          <h3 className="text-text-dark text-lg font-bold leading-tight tracking-[-0.015em]">Favori Kitapları</h3>
          <div className="flex gap-4 overflow-x-auto pb-4 -mx-5 px-5" style={{ scrollbarWidth: "none", MsOverflowStyle: "none" }}>
            <div className="flex-shrink-0 w-32">
        ...
        {/* 6. Detaylı Bilgiler */}
        <div className="space-y-4">
          <h3 className="text-text-dark text-lg font-bold leading-tight tracking-[-0.015em]">Detaylar</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div className="bg-surface-dark/50 p-3 rounded-lg flex items-center gap-3">
              <span className="material-symbols-outlined text-subtle-dark icon-outline">cake</span>
              <span className="text-text-dark text-sm">Terazi</span>
            </div>
            <div className="bg-surface-dark/50 p-3 rounded-lg flex items-center gap-3">
              <span className="material-symbols-outlined text-subtle-dark icon-outline">school</span>
              <span className="text-text-dark text-sm">Lisans</span>
            </div>
            <div className="bg-surface-dark/50 p-3 rounded-lg flex items-center gap-3">
              <span className="material-symbols-outlined text-subtle-dark icon-outline">music_note</span>
              <span className="text-text-dark text-sm">Indie Rock</span>
            </div>
            <div className="bg-surface-dark/50 p-3 rounded-lg flex items-center gap-3">
              <span className="material-symbols-outlined text-subtle-dark icon-outline">sports_esports</span>
              <span className="text-text-dark text-sm">Oyun</span>
            </div>
            <div className="bg-surface-dark/50 p-3 rounded-lg flex items-center gap-3">
              <span className="material-symbols-outlined text-subtle-dark icon-outline">flight</span>
              <span className="text-text-dark text-sm">Seyahat</span>
            </div>
            <div className="bg-surface-dark/50 p-3 rounded-lg flex items-center gap-3">
              <span className="material-symbols-outlined text-subtle-dark icon-outline">local_bar</span>
              <span className="text-text-dark text-sm">Bazen</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {isDrawer && (
        <div className="sticky bottom-0 left-0 right-0 bg-gradient-to-t from-background-dark via-background-dark to-transparent p-4 z-20">
          <div className="max-w-md mx-auto flex justify-around items-center gap-4 bg-background-dark/80 backdrop-blur-lg p-3 rounded-full border border-border-dark">
            <Button className="w-14 h-14 rounded-full bg-surface-dark/50 flex items-center justify-center text-subtle-dark hover:bg-surface-dark/70 transition-colors">
